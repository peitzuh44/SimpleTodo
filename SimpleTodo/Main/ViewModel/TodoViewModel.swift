...
class TodoViewModel: ObservableObject {
    ...
    func fetchTodo() {
        listenerRegistration = db.collection("todos").addSnapshotListener { [weak self] querySnapshot, error in
    ...
    func addTodo(name: String, due: Date) async {
        ...
        let todo = Todo(id: todoID, timeCreated: Date(), name: name, due: due, isDone: false)
        do {
            try
